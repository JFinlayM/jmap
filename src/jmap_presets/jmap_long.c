...
static void print_element_array_callback(const void *x){
    printf("%ld ", JMAP_GET_VALUE(const long, x));
}

static char *element_to_string_array_callback(const void *x){
    long value = JMAP_GET_VALUE(const long, x);
...
static bool is_equal_array_callback(const void *x, const void *y){
    return JMAP_GET_VALUE(const long, x) == JMAP_GET_VALUE(const long, y);
}


JMAP create_jmap_long(void){
    JMAP array;
    ...
    jmap.init(&array, sizeof(long), JMAP_TYPE_VALUE, imp);
    return array;
}